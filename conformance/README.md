...
- `fixtures/invalid.json`: 17 historical invalid imports covering mode/dimension mismatches, ranges, unknown fields, impossible timers, malformed geometry and underflowed selection weights. Revision 2 rejects 16; the .01-window profile is intentionally valid under broader bounds.
...
Revision 2 explicitly migrates original state metadata in the harness; numerical
checkpoints and original trajectory hashes are unchanged. `fixtures/extensions.json`
adds hand-derived nibble, early-hook, timeout and adjacent-surge checks.
`node conformance/check_extensions.mjs` compares complete Python/Rust-WASM states,
events and observations across all five target rules, three dimensions, boundary
seeds and long waits. Inputs are generated once by Python and replayed unchanged.
This differential check supplements, rather than regenerates, the golden corpus.
